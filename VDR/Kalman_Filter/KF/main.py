...
import cdf
import numpy as np
from VDR.Kalman_Filter.KF.IntegratedLocation import IntegratedLocation
...
for line in RTK:
    rtk_data = line.split(',')
    RTK_DATAS[float(rtk_data[0])] = [float(rtk_data[1]), float(rtk_data[2])]
...
for i in range(start_index):
    ...
    if datas[i][-1] != last_UTC:
        last_UTC = datas[i][-1]
        Changed = True
    result = il.deal_data(datas[i], Changed, RTK_DATAS)
...
#         Changed = False
#         result = temp.deal_data(datas[i], Changed, RTK_DATAS)
#     if datas[i][-9] != 0:
