...
class IntegratedLocation(object):
    ...
    def deal_data(self, datas, Changed, RTK):
        ...
        if self.firstGPSOff == 0:
            ...
            delta_Vx = self.LineFn[0, 0] * self.samplePeriod
            delta_Vy = self.LineFn[1, 0] * self.samplePeriod
            ...
            delta_Px = (self.lastVx / (math.cos(self.last_L) * self.Rn)) * self.samplePeriod
            delta_Py = (self.lastVy / self.Rm) * self.samplePeriod
            ...
            if GPSHDOP < 3.0 and Changed and isUsed:
                ...
                Dpv = np.mat([
                    ...
                    self.lastVx - self.GPSVe, self.lastVy - self.GPSVn, self.lastVz - self.GPSVu]).reshape(-1, 1)
